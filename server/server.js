...
setInterval(async () => {
  ...
  categories.forEach(async (category) => {
    if (postsCache[category] && Object.keys(postsCache[category]).length > 0) {
      ...
      if (postId && Math.random() <= COMMENT_POST_CHANCE && postsCache[category][postId].ai_summary && parseInt(postsCache[category][postId].comment_count) < 1) {
     ...
     //   console.log('created comment for ' + post.title);
        ...
        // Add random upvotes
        const min = -1, max = 10, randomUpvotes = Math.floor(Math.random() * (max - min + 1)) + min;
